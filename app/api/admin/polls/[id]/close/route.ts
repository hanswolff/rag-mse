...
export const POST = withApiErrorHandling(async (request: NextRequest, context: RouteParams) => {
  ...
  if (!poll) {
    logResourceNotFound("poll", id, "/api/admin/polls/[id]/close", "POST");
    return NextResponse.json({ error: "Umfrage nicht gefunden" }, { status: 404 });
  }

  if (poll.status !== "LIVE") {
    return NextResponse.json(
      { error: "Nur aktive Umfragen können geschlossen werden" },
      { status: 409 }
    );
  }
  ...
  logInfo("poll_closed", "Poll closed", { pollId: id, userId: user.id });

  return NextResponse.json(updated);
}, { route: "/api/admin/polls/[id]/close", method: "POST" });
